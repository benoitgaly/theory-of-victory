...
window.tovHexMap = (function () {
    ...
    function legend(svg, kmPerPixel) {
        var x = 16, y = 372;
        svg.appendChild(svgEl("rect", {
            x: x, y: y, width: 194, height: 122, rx: 6,
            fill: "#fffdf8", stroke: "#ded8ca", "stroke-width": "1", opacity: "0.94"
        }));

        var rows = [
            { label: "Tenu", fill: COLOUR.land, hatch: null, stroke: COLOUR.grid },
            { label: "Occupé avant février 2022", fill: "rgba(168,50,42,0.28)", hatch: null, stroke: COLOUR.grid },
            { label: "Pris depuis février 2022", fill: "rgba(168,50,42,0.12)", hatch: "ru", stroke: COLOUR.grid },
            { label: "Repris par l'Ukraine", fill: "rgba(30,95,168,0.12)", hatch: "ua", stroke: COLOUR.grid },
            { label: "Traversé par la ligne", fill: "rgba(168,50,42,0.14)", hatch: null, stroke: COLOUR.ru }
        ];

        rows.forEach(function (row, i) {
            var cy = y + 17 + i * 17.5, cx = x + 17, r = 6.4;
            var d = "";
            for (var k = 0; k < 6; k++) {
                var a = k * Math.PI / 3;
                d += (k === 0 ? "M" : "L") + (cx + r * Math.sin(a)).toFixed(1) + " " + (cy - r * Math.cos(a)).toFixed(1);
            }
            d += "Z";
            ...
            svg.appendChild(text(cx + 13, cy + 3.4, row.label, {
                "font-size": "9.5", fill: "#4a5259"
            }));
        });

        // Scale bar, 100 km.
        var barY = y + 108, barX = x + 14, barW = 100 / kmPerPixel;
        svg.appendChild(svgEl("path", {
            d: "M" + barX + " " + (barY - 4) + "V" + barY + "H" + (barX + barW) + "V" + (barY - 4),
            fill: "none", stroke: "#5c6470", "stroke-width": "1.2"
        }));
        svg.appendChild(text(barX + barW + 7, barY + 3, "100 km · 1 hex ≈ " + HEX_KM + " km", {
